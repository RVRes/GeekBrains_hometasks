...
class FormFeedback {
    constructor(formFB, inptName, inptPhone, inptEmail, inptText, selectCity, inptAll, borderRedStyle) {
        this.formFB = formFB;
        ...
        this.citiesLink = 'russian-cities.json';
        this._init();
    }

    _init() {
        fetch(this.citiesLink)
            .then(result => result.json())
            .then(data => {
                for (let city of data) {
                    let newCityItem = document.createElement('option');
                    newCityItem.value = city.city;
                    newCityItem.innerHTML = `${city.city}`;
                    this.selectCity.appendChild(newCityItem);
                }
            })
    }
    ...
    checkEmptyFields() {
        ...
        this.inptAll.forEach(function (item) {
            ...
            if (!item.value) {
                ...
                $(item.parentElement).effect("bounce", {}, 2000);
                result = false;
            }
        });
        return result;
    ...
    _checkInput(pattern, input) {
        ...
        if (!result) {
            ...
            $(input.parentElement).effect("bounce", {}, 2000);
            result = false;
        }
        return result;
    }
